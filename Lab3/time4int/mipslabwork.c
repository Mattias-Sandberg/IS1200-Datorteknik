...
void user_isr( void )
{
  if(IFS(0) & 0x100) // kollar efter timeout event flag T2
  {
  ...
  if(IFS(0) & (1 << 11)) // För att få switch 2 att fungera rätt
  {
    IFS(0) = 0; // sätter IFS till 0
    mytime += 0x0003; // utökar mytime med 3
  }
}
...
void labinit( void )
{
 ...
 /* Period register, Sätter värdet till (klock frekvens)/(prescaling)
  ...
  //eneblar interrupts för SW2
  IECSET(0) = 0x800; //int 2 bit 11 (interrupt enable control set)
  IPC(3) = 0x1c000000; // Sätter prioritet
