...
tiend:	sw	$t0,0($a0)	# save updated result
  ...
  delay: 
	...
	while:
		ble 	$t1, $zero, exit_delay	# kollar om $t1 (ms) är "branch less than" 0. om ms är mindre än noll, hoppa till exit_delay
		nop				# Slut på operation
		sub	$t1, $t1, 1 		# ms--, subtraherar 1 från ms efter varje "loop"
		
	li	$t2, 0				# Load immediate (int $t2 = 0)	
	for:
		bgt  	$t2, 50000, while	# kolla om 0 < andvändar input (ms), om det stämmer kör loopen annars hoppa till exit_delay
		nop				# Slut på operation
